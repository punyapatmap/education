...
course.path = "courses/shee/sharing/"

files <- list.files(full.names = F, pattern = "\\.qmd", recursive = T)
files <- files[grep("main_sharing.qmd", files, invert = TRUE)]
...
data.frame(Date    = lectures$date, 
           ...
           Presentation  = sprintf('<a href="https://punyapatmap.github.io/education/%s" target="_blank" class="external">link</a>', gsub("qmd", "slide.html", lectures$url))
) -> lecture.list
